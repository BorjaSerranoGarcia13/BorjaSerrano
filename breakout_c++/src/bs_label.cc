...
void BsLabel::init(const char* data, const char* font_src, uint32_t size){
  if (size == 0) {
	  std::cout <<"LABEL size 0";
    return;
  }

  if (NULL == data) {
	  std::cout <<"LABEL data NULL";
    return;
  }

  if (NULL == font_src) {
	  std::cout <<"LABEL font NULL";
    return;
  }

  if (!font_.loadFromFile(font_src)) {
	  std::cout <<"LABEL no font found";
    return;
  }
...
void BsLabel::init(const char* data,const char* font_src, uint32_t size, sf::Vector2f position, float rotation, sf::Vector2f scale, uint8_t border_color[4], uint8_t fill_color[4]) {
