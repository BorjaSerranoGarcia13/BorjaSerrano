...
//AMBIENT
  ...
  vec3 ambient = u_ambientStrength * base_color.rgb;
...
vec3 norm = normalize(normal);
  ...
  vec3 diffuse = diff * base_color.xyz;
  //SPECULAR
  ...
  // SPOT CALCULATIONS
   ...
   //POINTLIGHT
   vec3 pointlight_position = vec3(4.0f, 1.0f, 0.0f);
  ...
  fragColor = mix(vec4(0.5f, 0.5f, 0.5f, 0.5f), fragColor , visibility);
  //fragColor = vec4(result, 1.0f);
}
