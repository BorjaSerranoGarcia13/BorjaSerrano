...
//view
void material_transparent::setupCamera(const float projection[16], const float m[16]) const {
...
bool material_transparent::enable(const EDK3::MaterialSettings * settings) const {
	...
	const material_transparent::Settings* sett = dynamic_cast<const material_transparent::Settings*>(settings);
	//sett->texture()->bind(0);
	...
	program_->set_uniform_value(program_->get_uniform_position("campos"), EDK3::T_FLOAT_3, campos);
