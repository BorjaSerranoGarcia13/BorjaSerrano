...
#include <ESAT/time.h>
																	...
																	//view
void custom_material::setupCamera(const float projection[16], const float m[16]) const {
	...
	position = program_.get()->get_uniform_position("u_v_matrix");
	program_.get()->set_uniform_value(position, EDK3::Type::T_MAT_4x4, m);

	position = program_.get()->get_uniform_position("u_p_matrix");
	program_.get()->set_uniform_value(position, EDK3::Type::T_MAT_4x4, projection);
...
unsigned int custom_material::num_attributes_required() const {
	return 3;
}
void custom_material::setupModel(const float m[16]) const {
	int position = program_.get()->get_uniform_position("u_m_matrix");
	program_.get()->set_uniform_value(position, EDK3::Type::T_MAT_4x4, m);
}

EDK3::Attribute custom_material::attribute_at_index(const unsigned int attrib_idx) const {
...
EDK3::Type custom_material::attribute_type_at_index(const unsigned int attrib_index) const {
...
custom_material::custom_material() {
	...
	loadVertexShaderFile(&vertShader, "test.vert");
	loadFragmentShaderFile(&fragShader, "test.frag");
...
bool custom_material::enable(const EDK3::MaterialSettings * settings) const {

	const custom_material::Settings* sett = dynamic_cast<const custom_material::Settings*>(settings);
	sett->texture()->bind(0);
	...
	program_->set_uniform_value(program_->get_uniform_position("u_density"), EDK3::T_FLOAT_1, &sett->density_);
	program_->set_uniform_value(program_->get_uniform_position("u_gradient"), EDK3::T_FLOAT_1, &sett->gradient_);
	program_->set_uniform_value(program_->get_uniform_position("lightPos"), EDK3::T_FLOAT_1, &value);
	...
	program_->set_uniform_value(program_->get_uniform_position("u_ambientStrength"), EDK3::T_FLOAT_1, &sett->lightAmbient_);
