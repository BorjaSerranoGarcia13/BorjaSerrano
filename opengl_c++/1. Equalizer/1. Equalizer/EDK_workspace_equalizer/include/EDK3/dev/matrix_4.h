...
inline Matrix4x4 Matrix4x4::Translate(const Vector3& distance) {
	...
	tmp[4] = 0.0f;tmp[5] = 1.0f;tmp[6] = 0.0f;tmp[7] = 0.0f;
	tmp[8] = 0.0f;tmp[9] = 0.0f;tmp[10] = 1.0f;tmp[11] = 0.0f;
	tmp[12] = distance.x;tmp[13] = distance.y;tmp[14] = distance.z;tmp[15] = 1.0f;
	return Matrix4x4(tmp);
}
inline Matrix4x4 Matrix4x4::Translate(float x, float y, float z){
	...
	tmp[4] = 0.0f;tmp[5] = 1.0f;tmp[6] = 0.0f;tmp[7] = 0.0f;
	tmp[8] = 0.0f;tmp[9] = 0.0f;tmp[10] = 1.0f;tmp[11] = 0.0f;
	tmp[12] = x;tmp[13] = y;tmp[14] = z;tmp[15] = 1.0f;
	return Matrix4x4(tmp);
}
...
inline Matrix4x4 Matrix4x4::GetTransform(const Vector3& translate,
	...
	Matrix4x4 tmp;
	tmp.Identity();
	tmp.Multiply(Scale(scale));
	tmp.Multiply(RotateX(rotateX));
	tmp.Multiply(RotateY(rotateY));
	tmp.Multiply(RotateZ(rotateZ));
	tmp.Multiply(Translate(translate));
	return Matrix4x4(tmp);
}
...
inline Matrix4x4 Matrix4x4::GetTransform(float trans_x, float trans_y, float trans_z,
	...
	Matrix4x4 tmp;
	tmp.Identity();
	tmp.Multiply(Scale({ scale_x, scale_y, scale_z }));
	tmp.Multiply(RotateX(rotateX));
	tmp.Multiply(RotateY(rotateY));
	tmp.Multiply(RotateZ(rotateZ));
	tmp.Multiply(Translate({ trans_x, trans_y, trans_z }));
	return Matrix4x4(tmp);
}
...
inline Matrix4x4 Matrix4x4::PerspectiveMatrix(float fov, float aspect,
	float near, float far) const {
	float xymax = near * tan(fov * 0.0087266f);
	float ymin = -xymax;
	float xmin = -xymax;
