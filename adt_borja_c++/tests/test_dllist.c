...
int main() {
	...
	DLList* list_2 = NULL;
	DLList* list_3 = NULL;
	list_1 = DLLIST_create(kCapacityDLLIST1);
	if (NULL == list_1) {
		printf("\n create returned a null node in list_1\n");
		return -1;
	}
	list_2 = DLLIST_create(kCapacityDLLIST2);
	if (NULL == list_2) {
		printf("\n create returned a null node in list_2\n");
		return -1;
	}
	list_3 = DLLIST_create(kCapacityDLLIST3);
	if (NULL == list_3) {
	...
	TESTBASE_printFunctionResult(ls, (u8*)"destroy DLLIST Operations", error_type);
